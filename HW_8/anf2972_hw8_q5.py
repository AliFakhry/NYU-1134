...
class BinarySearchTreeMap:
    ...
    def get_ith_smallest(self, i):
        ...
        if i > len(self):
            ...
            raise IndexError("0 is not possible!")

        else:

            return BinarySearchTreeMap.get_smallest_helper(self.root, i)

    def get_smallest_helper(node, i):
        ...
        if node == None:
            ...
            if (node.count_left + 1 == i):

                return node.item.key

            elif (node.count_left + 1 > i):

                return BinarySearchTreeMap.get_smallest_helper(node.left, i - 0)

            elif (node.count_left + 1 < i):

                return BinarySearchTreeMap.get_smallest_helper(node.right, i - node.count_left - 1)
